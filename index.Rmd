 ...
 * **What to prepare?** Please bring your laptop computer! If possible, install R and RStudio prior to the course (see [links](Links.html) or instructions below). 
...
[Various R cheatsheets](https://posit.co/resources/cheatsheets/?type=posit-cheatsheets&_page=2/) (I recommend the following cheatsheets for this course:    
...
To ensure we maximize time during the course please also download the data files for this course from the [Data](data.html) tab and save this in a folder on your hard drive. *We will move these to a specific folder for the course when we cover folder organization in the first module*

Right click (or command-click for MAcs) each file. The .txt files will open in a viewer window and you will need to right click to save them. The .csv files should download directly in your downloads folder and you can move them where you want to save them. **Important DO NOT change the file names** the file names should match what is shown in () after the file description.
...
[--go to first module](mod_1_R101.html)
...
```{r echo=FALSE}
rmd2rscript <- function(infile="mod_4_data_manipulation.Rmd"){    # function for converting markdown to scripts
